...
def units_table() -> str:
    ...
    rows = ["| Type | Label | Confidence | Move | Speed | Action | Cost | ETA (s) | Capacity | Spray r | Cut w | Notes |",
            "|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for k, v in d.items():
        notes = []
        if v.get("passenger_slots"): notes.append(f"carries {v['passenger_slots']}")
        if v.get("reload_at_base"): notes.append(f"reloads at base {v.get('reload_seconds', 0):.0f}s")
        if v.get("hose_max_length"): notes.append(f"hose ≤ {v['hose_max_length']} cells")
        if v.get("can_cut_dense"): notes.append("cuts dense forest")
        if v.get("drop_agent"): notes.append(f"drops {v['drop_agent']} ×{v.get('drop_width')} wide")
        if v.get("is_civilian"): notes.append("rescue objective")
        if v.get("air_deployable"): notes.append("air-deployable")
        rows.append(f"| `{k}` | {v.get('label', k)} | {v.get('confidence', '')} | {v['movement']} | {v['speed']} | "
...
def terrain_table() -> str:
    ...
    rows = ["| Terrain | Letter | Fuel | Ignition rate | Burn rate | Heat | Smolder (s) | FOOT | ROAD | OFFROAD | AIR | WATERCRAFT |",
            "|---|---|---|---|---|---|---|---|---|---|---|---|"]
    letters = {"WATER": "W", "GRASS": "G", "SHRUB": "S", "FOREST": "F", "DENSE_FOREST": "D", "ROAD": "R",
               "GRAVEL": "V", "STRUCTURE": "B", "FIREBREAK": "X", "SAND": "A"}
    for k, v in sorted(d.items(), key=lambda kv: kv[1]["id"]):
        m = v["move"]
        mv = lambda c: "—" if m.get(c) is None else str(m[c])
        rows.append(f"| `{k}` | {letters.get(k, '?')} | {v['fuel']} | {v['ignition_rate']} | {v['burn_rate']} | "
