...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="backburn", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    r.add_argument("scenario"); r.add_argument("--ticks", type=int, default=600)
    r.add_argument("--gif"); r.add_argument("--png"); r.add_argument("--every", type=int, default=10)
    r.add_argument("--scale", type=int, default=4); r.add_argument("--overlay", choices=["heat", "moisture", "elevation"])
    r.add_argument("--save", help="write a .bbsave at the end")

    rp = sub.add_parser("replay", help="re-run a replay file")
    rp.add_argument("replay"); rp.add_argument("--png"); rp.add_argument("--until", type=int)

    rs = sub.add_parser("resume", help="continue from a savegame headlessly")
    rs.add_argument("save"); rs.add_argument("--ticks", type=int, default=600); rs.add_argument("--png")
    ...
    g.add_argument("out"); g.add_argument("--seed", type=int, default=1)
    ...
    b.add_argument("scenario", nargs="?", default=str(DEFAULT)); b.add_argument("--ticks", type=int, default=300)
...
def _dispatch(a) -> int:
    ...
    if a.cmd == "validate":
        ...
        for f in a.files:
            try:
                s = load_scenario(f)
                print(f"OK    {f}  ({s.name}, {s.width}×{s.height}, {len(s.units)} units, {len(s.ignitions)} ignitions)")
            except ScenarioError as e:
                bad += 1
                print(f"ERROR {e}")
    ...
    if a.cmd == "bench":
        sim = Simulation(load_scenario(a.scenario))
        t0 = time.perf_counter(); n = sim.step(a.ticks); dt = time.perf_counter() - t0
        print(f"{n} ticks in {dt:.2f}s  →  {n / dt:.0f} ticks/s  ({n / dt / 10:.0f}× real time at 10 ticks/s)")
        return 0

    if a.cmd == "info":
        s = load_scenario(a.scenario)
        print(f"{s.name}\n  size {s.width}×{s.height}  seed {s.seed}  moisture {s.moisture}  duration {s.duration:.0f}s")
        print(f"  wind {s.wind_speed} m/s toward {s.wind_bearing}°  terrain {s.terrain_mode}  elevation {'yes' if s.elevation else 'no'}")
        print(f"  ignitions {len(s.ignitions)}  units {len(s.units)}  events {len(s.events)}  budget {s.budget}")
...
def _report(sim: Simulation) -> None:
    st = sim.stats()
    print(f"t={st['time']:.0f}s outcome={st['outcome']} burning={st['burning']} burned={st['area_burned_pct']}% "
