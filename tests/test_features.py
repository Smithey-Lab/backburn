...
from backburn.config import FIRE, UNITS, TerrainType as T
from backburn.fire import BURNING, FireGrid
...
def rescue_scenario():
    t = flat(T.GRASS, 80, 80); t[:, 0:4] = int(T.WATER)
    return Scenario(name="rescue", width=80, height=80, terrain_mode="grid", terrain_grid=t,
                    wind_speed=6, wind_bearing=90, airbase=(5, 5), staging=(6, 40), safe_zone=(5, 10, 4),
                    units=[{"type": "HELICOPTER", "x": 5, "y": 5}, {"type": "CIVILIAN", "x": 30, "y": 30},
                           {"type": "CIVILIAN", "x": 31, "y": 31}],
                    ignitions=[{"x": 12, "y": 30, "radius": 1}], duration=900,
...
def test_civilians_flee_and_can_die():
    ...
    sc = Scenario(width=40, height=40, terrain_mode="grid", terrain_grid=t, wind_speed=8, wind_bearing=90,
                  units=[{"type": "CIVILIAN", "x": 20, "y": 20}], ignitions=[{"x": 10, "y": 20, "radius": 2}])
    sim = Simulation(sc)
    c = sim.world.units[0]
    x0 = c.x
    sim.step(40)
...
def test_budget_blocks_purchase_and_dispatch_delays_arrival():
    sc = rescue_scenario(); sc.budget = 1000; sc.units = []
    sim = Simulation(sc)
...
def test_unavailable_unit_is_refused():
    sc = rescue_scenario(); sc.available_units = ["ENGINE"]
    sim = Simulation(sc)
...
def test_events_fire_once_at_their_time():
    sc = rescue_scenario(); sc.units = []
    sc.events = [{"at": 30, "wind": {"speed": 12, "bearing": 180}}, {"at": 50, "message": "hello"},
                 {"at": 70, "spawn": {"type": "ENGINE"}}, {"at": 90, "ignite": {"x": 60, "y": 60}}]
    sim = Simulation(sc)
    sim.step(29); assert sim.grid.wind_speed == 6
    sim.step(2);  assert sim.grid.wind_speed == 12 and sim.grid.wind_bearing == 180
    sim.step(70)
...
def test_contained_outcome_and_score():
    t = flat(T.GRASS, 40, 40); t[:, 20] = int(T.WATER)   # small burnable pocket on the left
    t[:, 0:5] = int(T.WATER)
    sc = Scenario(width=40, height=40, terrain_mode="grid", terrain_grid=t, ignitions=[{"x": 10, "y": 20, "radius": 1}],
                  duration=3000)
    sim = Simulation(sc)
    sim.step(3000)
...
def test_failed_on_structure_limit():
    t = flat(T.GRASS, 40, 40); t[20:22, 25:27] = int(T.STRUCTURE)
    sc = Scenario(width=40, height=40, terrain_mode="grid", terrain_grid=t, wind_speed=6, wind_bearing=90,
                  ignitions=[{"x": 10, "y": 20, "radius": 1}], objectives={"max_structures_lost": 0}, duration=2000)
    sim = Simulation(sc)
    sim.step(2000)
...
def test_timeout_outcome():
    ...
    sc = Scenario(width=40, height=40, terrain_mode="grid", terrain_grid=t, ignitions=[{"x": 20, "y": 20, "radius": 2}],
                  duration=60)
    sim = Simulation(sc)
    n = sim.step(500)
...
def test_step_stops_after_outcome():
    t = flat(T.GRASS, 30, 30); t[:, 15] = int(T.WATER)
    sc = Scenario(width=30, height=30, terrain_mode="grid", terrain_grid=t, ignitions=[{"x": 5, "y": 15}], duration=5000)
    sim = Simulation(sc)
    sim.step(5000)
...
def test_spotting_jumps_a_break_only_in_high_wind():
    t = flat(T.FOREST, 64, 128); t[:, 60:63] = int(T.FIREBREAK)
    crossed = {}
    for wind in (4.0, 18.0):
        g = FireGrid(t, seed=5); g.set_wind(wind, 90); g.ignite(20, 32, 2)
        for _ in range(500):
            g.step()
...
def test_no_spotting_below_threshold_wind():
    ...
    g = FireGrid(t, seed=2); g.set_wind(float(FIRE["spot_min_wind"]) - 1, 90); g.ignite(10, 24, 2)
    for _ in range(200):
        g.step()
...
def test_fire_runs_faster_uphill():
    ...
    up = FireGrid(t, seed=3, elevation=ramp); up.ignite(64, 32, 1)
    down = FireGrid(t, seed=3, elevation=ramp[:, ::-1].copy()); down.ignite(64, 32, 1)
    for _ in range(80):
        up.step(); down.step()
    ux = np.nonzero(up.state > 0)[1]; dx = np.nonzero(down.state > 0)[1]
...
def test_generated_elevation_is_deterministic():
    a = generate_elevation(32, 32, 9, relief=50.0); b = generate_elevation(32, 32, 9, relief=50.0)
...
def test_replay_with_purchases_and_pickup(tmp_path):
    ...
    sim.step(150)
    p = tmp_path / "r.json"; sim.save_replay(p)
...
def test_validation_messages_name_the_field():
    ...
    bad = dict(base); bad["units"] = [{"type": "TANK", "x": 1, "y": 1}]
    with pytest.raises(ScenarioError, match="units\\[0\\].type"):
        validate(bad)
    bad = dict(base); bad["ignitions"] = [{"x": 9999, "y": 1}]
    with pytest.raises(ScenarioError, match="outside"):
        validate(bad)
    bad = dict(base); bad["terrain"] = {"mode": "grid", "rows": ["GGG", "GG"]}
    with pytest.raises(ScenarioError, match="length"):
        validate(bad)
    bad = dict(base); bad["objectives"] = {"win_if_cool": True}
...
def test_cli_run_and_replay(tmp_path, capsys):
    from backburn.__main__ import main
    png = tmp_path / "f.png"; save = tmp_path / "s.bbsave"
    assert main(["run", str(ROOT / "scenarios" / "prairie_fire.json"), "--ticks", "40", "--png", str(png), "--save", str(save)]) == 0
