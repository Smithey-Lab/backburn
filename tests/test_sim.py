...
from backburn.config import MoveClass, TERRAIN, TerrainType as T
...
def test_same_seed_same_hash():
    a = FireGrid(flat(T.GRASS), seed=42); b = FireGrid(flat(T.GRASS), seed=42)
    for g in (a, b):
        g.set_wind(5, 90); g.ignite(20, 48, 1)
        for _ in range(150):
            g.step()
...
def test_different_seed_different_hash():
    a = FireGrid(flat(T.GRASS), seed=1); b = FireGrid(flat(T.GRASS), seed=2)
    for g in (a, b):
        g.ignite(20, 48, 1)
        for _ in range(100):
            g.step()
...
def test_wind_biases_spread_downwind():
    g = FireGrid(flat(T.GRASS), seed=3); g.set_wind(6, 90); g.ignite(30, 48, 1)
    for _ in range(120):
        g.step()
...
def test_no_wind_is_roughly_isotropic():
    g = FireGrid(flat(T.GRASS), seed=3); g.set_wind(0, 0); g.ignite(48, 48, 1)
    for _ in range(100):
        g.step()
    ys, xs = np.nonzero(g.state > 0)
    ex = (xs.max() - 48, 48 - xs.min()); ey = (ys.max() - 48, 48 - ys.min())
...
def test_firebreak_holds_without_spotting():
    t = flat(T.GRASS); t[:, 50] = int(T.FIREBREAK)
    g = FireGrid(t, seed=3); g.set_wind(12, 90); g.ignite(20, 48, 1)
    for _ in range(400):
        g.step()
...
def test_road_and_water_do_not_burn():
    t = flat(T.GRASS); t[:, 40:43] = int(T.ROAD); t[:, 60:70] = int(T.WATER)
    g = FireGrid(t, seed=3); g.ignite(20, 48, 2)
    for _ in range(300):
        g.step()
...
def test_water_extinguishes_and_leaves_fuel():
    g = FireGrid(flat(T.GRASS, 64, 64), seed=3); g.ignite(32, 32, 2)
    for _ in range(15):
        g.step()
    before = g.stats().burning
    g.apply_water(32, 32, 6, 1.0); g.step()
...
def test_retardant_blocks_ignition():
    ...
    g = FireGrid(t, seed=3); g.set_wind(6, 90); g.ignite(20, 32, 1)
...
def test_burn_lifecycle():
    g = FireGrid(flat(T.GRASS, 16, 16), seed=1); g.ignite(8, 8)
    seen = set()
    for _ in range(80):
        g.step(); seen.add(int(g.state[8, 8]))
...
def test_engine_cannot_enter_forest_but_brush_truck_can():
    t = flat(T.GRASS, 32, 32); t[:, 15:18] = int(T.FOREST)
    st = np.zeros_like(t)
...
def test_path_avoids_fire():
    ...
    g = FireGrid(t, seed=1)
    g.state[:, 15] = BURNING; g.state[0:10, 15] = UNBURNED  # gap at top
...
def test_units_snap_to_passable_and_move():
    t = flat(T.GRASS, 48, 48); t[20, :] = int(T.ROAD); t[:15, :] = int(T.DENSE_FOREST)
    sc = Scenario(width=48, height=48, terrain_mode="grid", terrain_grid=t,
                  units=[{"type": "ENGINE", "x": 5, "y": 5}])
    sim = Simulation(sc)
    e = sim.world.units[0]
...
def test_hose_team_connects_and_burns():
    t = flat(T.GRASS, 48, 48); t[:, 0:3] = int(T.WATER)
    sc = Scenario(width=48, height=48, terrain_mode="grid", terrain_grid=t,
...
def test_helicopter_drops_and_refills():
    t = flat(T.GRASS, 48, 48); t[0:6, 0:6] = int(T.WATER)
    sc = Scenario(width=48, height=48, terrain_mode="grid", terrain_grid=t,
...
def test_scenario_roundtrip_grid_mode(tmp_path):
    ...
    sc = Scenario(name="rt", width=40, height=30, terrain_mode="grid", terrain_grid=t,
                  ignitions=[{"x": 3, "y": 3}], units=[{"type": "ENGINE", "x": 1, "y": 1}])
...
def test_perf_budget():
    ...
    import time
    sim = Simulation(load_scenario(ROOT / "scenarios" / "prairie_fire.json"))
    t0 = time.time(); sim.step(200); dt = time.time() - t0
